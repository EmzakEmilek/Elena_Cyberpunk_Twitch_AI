...
class Elena:
    ...
    def _setup_keyboard_listener(self):
        ...
        def on_press(key):
            if key == keyboard.Key.f12:  # TODO: Konfigurovateľné
                self.audio.start_recording()
                # Vyčisti posledný riadok a zobraz indikátor nahrávania
        ...
        try:
            ...
            if text:
                ...
                if response:
                    ...
                    trans_color = Fore.GREEN if transcription_time < 2.0 else (Fore.YELLOW if transcription_time < 4.0 else Fore.RED)
                    assist_color = Fore.GREEN if assistant_time < 3.0 else (Fore.YELLOW if assistant_time < 6.0 else Fore.RED)
                    total_color = Fore.GREEN if total_time < 5.0 else (Fore.YELLOW if total_time < 8.0 else Fore.RED)
                    
                    print(f"  • Transkripcia: {trans_color}{transcription_time:.1f}s{Style.RESET_ALL}")
                    print(f"  • Odpoveď: {assist_color}{assistant_time:.1f}s{Style.RESET_ALL}")
                    print(f"  • Celkový čas: {total_color}{total_time:.1f}s{Style.RESET_ALL}")
    ...
    def run(self):
        ...
        try:
            ...
            print(f"{Fore.CYAN}┃{Style.RESET_ALL}           {Fore.WHITE}{Style.BRIGHT}Elena STT v2.0{Style.RESET_ALL}             {Fore.CYAN}┃{Style.RESET_ALL}")
            ...
            print(f"  • Stlač a drž {Fore.CYAN}{Style.BRIGHT}[F12]{Style.RESET_ALL} pre nahrávanie")
            print(f"  • Pusti {Fore.CYAN}{Style.BRIGHT}[F12]{Style.RESET_ALL} pre prepis")
            print(f"  • Stlač {Fore.CYAN}{Style.BRIGHT}[Ctrl+C]{Style.RESET_ALL} pre ukončenie\n")
